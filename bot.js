...
async function onMessage(message) {
    ...
    const isAdmin = String(message.from.id) === String(ADMIN_UID);
    if (text === '/begin' && !isAdmin) {
        ...
        try {
            ...
            startMsg = '获取开始消息时发生错误。';
        }
        startMsg = startMsg
            .replace(/{{username}}/g, `\`${username}\``)
            .replace(/{{user_id}}/g, `\`${userID}\``)
            .replace(/{{user}}/g, user);
        await sBot.put('user-started-' + userID, true);
        return sendMessage({
            ...
            reply_markup: {
                inline_keyboard: [[
                    { text: 'AiMi的店铺', url: 'https://qunimabi.de' }
                ]]
            }
        });
    }
    if (!isAdmin) {
        const hasStarted = await sBot.get('user-started-' + userID, { type: 'json' });
        if (!hasStarted) {
            return sendMessage({
                chat_id: message.chat.id,
                text: '先发送 /begin 再发消息。'
    ...
    if (isAdmin) {
        ...
        if (!message?.reply_to_message?.chat) {
            return sendMessage({
                chat_id: ADMIN_UID,
                text: '请回复用户消息后再操作 `/block` `/unblock` `/checkblock`'
            });
        }
        if (/^\/block(\s|@)?/.test(text)) return handleBlock(message);
        if (/^\/unblock(\s|@)?/.test(text)) return handleUnBlock(message);
        if (/^\/checkblock(\s|@)?/.test(text)) return checkBlock(message);
