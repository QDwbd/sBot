...
3. 找到一个群组.从[@nmnmfunbot](https://t.me/nmnmfunbot)获取自己的id
...
6. 先在Workers KV创建一个Namespace为`sBot`的KV 命名空间，之后在worker的Bindings(绑定)中添加`KV Namespace Bindings(KV命名空间)`：sBot -> sBot
7. 回到worker，复制[这个文件](./bot.js)到编辑器中保存
8. 通过打开`https://你的worker域名/registerWebhook`来注册websoket
```
registerWebhook
```
...
2. 从[uuidgenerator](https://www.uuidgenerator.net/)获取一个随机uuid作为secret
3. 创建一个群并改为话题模式.从[@nmnmfunbot](https://t.me/nmnmfunbot)获取你的群id和用户id.管理员账号不能设为匿名管理！！！！！！
4. 登录[cloudflare](https://workers.cloudflare.com/)，创建一个worker
5. 在设置里配置worker的变量(变量和密钥.纯文本类型)
...
回到Worker复制[这个文件](./newbot.js)到编辑器粘贴并部署

设置 Webhook 地址且访问一次，并确保 Telegram Bot API 使用你的 ENV_BOT_SECRET 作为 secret_token
